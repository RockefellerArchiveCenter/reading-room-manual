...
Select digitized materials can be accessed via the [Virtual Vault](http://virtualvault.rockarch.org/). If such an item is requested, direct the researcher to [http://virtualvault.rockarch.org](http://virtualvault.rockarch.org) either verbally or by email. Refer to Aeon Guides for the RAC for instructions for the latter.

As needed, touch base with the retrievals staff using the two-way radio so they are aware of the retrievals and any delays with location information. (See S:\Inter-Office\Reference\Reading Room Information for instructions for the radios and staff retrieval schedule.) Speak clearly and slowly into the radio, and always be aware that the entire Reading Room can hear both ends of a conversation. Therefore, you may request over the radio that the retrieval staff member calls you on the phone if you need to discuss a retrieval. The radio is also the appropriate and preferred way to ask for a second cart that is on hold in Room 103 to be brought up to the Reading Room.
